...
import type { GstServiceItem } from "../../app/services/gst-consultation/data";
...
function ServiceCard({
  ...
  service: GstServiceItem;
  index: number;
  onRequest: (service: GstServiceItem) => void;
}) {
  return (
    <motion.article
      id={service.id}
...
export default function GstConsultationPage() {
  const { intro, whyChoosePoints, ...hero } = gstPageContent;
  const [activeService, setActiveService] = useState<GstServiceItem | null>(null);
  const [modalOpen, setModalOpen] = useState(false);

  const openEnquiry = (service: GstServiceItem) => {
    ...
    <main className="ngo-page">
      ...
      <section className="ngo-services">
        <div className="ngo-container">
          ...
          <nav className="ngo-toc" aria-label="Tax filing service categories">
            {gstServiceCategories.map((category) => (
