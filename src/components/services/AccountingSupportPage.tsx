...
import type { AccountingServiceItem } from "../../app/services/accounting-support/data";
...
function ServiceCard({
  ...
  service: AccountingServiceItem;
  index: number;
  onRequest: (service: AccountingServiceItem) => void;
}) {
  return (
    <motion.article
      id={service.id}
...
export default function AccountingSupportPage() {
  const { intro, whyChoosePoints, ...hero } = accountingPageContent;
  const [activeService, setActiveService] = useState<AccountingServiceItem | null>(null);
  const [modalOpen, setModalOpen] = useState(false);

  const openEnquiry = (service: AccountingServiceItem) => {
    ...
    <main className="ngo-page">
      ...
      <section className="ngo-services">
        <div className="ngo-container">
          ...
          <nav className="ngo-toc" aria-label="Accounting service categories">
            {accountingServiceCategories.map((category) => (
      ...
      <NgoServiceEnquiryModal service={activeService as any} open={modalOpen} onClose={closeEnquiry} />
    </main>
  );
}
