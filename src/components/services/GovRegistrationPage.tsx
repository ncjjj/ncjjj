...
function ServiceCard({ service, index, onRequest }: { service: any; index: number; onRequest: (s: any) => void }) {
  return (
    <motion.article
      id={service.id}
      ...
      <div className="ngo-service-card__columns">
        <div className="ngo-service-card__block">
          ...
          <ul className="ngo-service-card__list">
            {service.assistance.map((item: string) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
        </div>
...
export default function GovRegistrationPage() {
  const { intro, whyChoosePoints, ...hero } = regPageContent;
    ...
    <main className="ngo-page">
      <section className="ngo-hero">
        <div className="ngo-hero__inner">
          <motion.span className="ngo-hero__badge" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            {hero.badge}
          </motion.span>

          <motion.h1 className="ngo-hero__title" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.08 }}>
            {hero.title}
          </motion.h1>

          <motion.p className="ngo-hero__subtitle" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.14 }}>
            {hero.subtitle}
          </motion.p>
        </div>
      </section>
      ...
      <section className="ngo-services">
        <div className="ngo-container">
          <motion.h2 className="ngo-section-title" initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
            {hero.servicesHeading}
          </motion.h2>

          <nav className="ngo-toc" aria-label="Service categories">
            {regServiceCategories.map((category) => (
